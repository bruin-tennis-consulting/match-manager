...
export default function MatchManagement() {
    ...
    try {
      ...
      if (selectedCollection === 'UCLA (M)' && selectedMatch.pointsJson) {
        const updatedPointsJson = selectedMatch.pointsJson.map(point => ({
          ...point,
          ...(clientTeam ? { clientTeam: clientTeam } : {}),
          ...(opponentTeam ? { opponentTeam: opponentTeam } : {})
        }))
  ...
  const handleBatchUpdate = async () => {
    ...
    if (selectedCollection === 'UCLA (W)' && !newTeamName.includes('(W)')) {
      setMessage('For women\'s collection, the new team must be a women\'s team (W)')
      return
    }
    ...
    try {
      ...
      matchesToUpdate.forEach(match => {
        ...
        const updateData = batchUpdateType === 'client'
          ? { client: newTeamName }
      ...
      if (updateCount > 0) {
        ...
        setMatches(prev => 
          prev.map(match => {
            ...
            } else if (batchUpdateType === 'opponent' && match.opponent === oldTeamName) {
        ...
        setMessage(`No matches found with ${batchUpdateType === 'client' ? 'client' : 'opponent'} team "${oldTeamName}"`)
      }
  ...
  const getSuggestedTeam = (teamName, isClient = true) => {
    ...
    if (isClient) {
      ...
      const uclaTeams = teams.filter(team => 
      ...
      const nonUclaTeams = teams.filter(team => 
        !team.name.includes('UCLA') && 
        !team.name.includes('University of California, Los Angeles')
      )
      ...
      if (selectedCollection === 'UCLA (M)') {
        const menTeams = nonUclaTeams.filter(team => team.name.includes('(M)'))
        return menTeams[0]?.name || ''
      } else if (selectedCollection === 'UCLA (W)') {
        const womenTeams = nonUclaTeams.filter(team => team.name.includes('(W)'))
  ...
  const findSimilarTeams = (teamName) => {
    ...
    return teams
      .filter(team => {
        ...
        return lowercaseName.split(' ').some(part => 
          part.length > 2 && lowerTeamName.includes(part)
        )
      })
    ...
    <div className={styles.container}>
      ...
      {showBatchUpdate && (
        <div className={styles.batchUpdatePanel}>
          ...
          <p>Replace all occurrences of a team name in the current collection</p>
            ...
            <div className={styles.teamSelectContainer}>
              <div className={styles.teamSelectGroup}>
                ...
                >
                  ...
                  {Object.entries(batchUpdateType === 'client' ? clientCounts : opponentCounts)
              ...
              filteredMatches.map((match) => (
                ...
                >
                  <div className={styles.matchTeams}>
                    <span className={styles.teamName}>{match.client || 'Unknown team'}</span>
                    <span> vs </span>
                    <span className={styles.teamName}>{match.opponent || 'Unknown team'}</span>
                  </div>
        ...
        {selectedMatch && (
          <div className={styles.matchDetailsPanel}>
            ...
            <div className={styles.matchCard}>
              ...
              <div className={styles.teamsContainer}>
                <div className={styles.teamDisplay}>
                  ...
                  {selectedMatch.client ? (
                    <p className={styles.teamNameDisplay}>{selectedMatch.client}</p>
                  ) : (
                    <p className={styles.missingTeam}>Unknown team</p>
                  )}
                  
                  {/* Show suggestion if team is unknown */}
                  {(!selectedMatch.client || !teams.some(t => t.name === selectedMatch.client)) && (
                    <div className={styles.teamSuggestion}>
                      <p>Suggested: {getSuggestedTeam(selectedMatch.client, true)}</p>
                      <button 
                        className={styles.applySuggestion}
                        onClick={() => setClientTeam(getSuggestedTeam(selectedMatch.client, true))}
                      >
                        Apply Suggestion
                      </button>
                      ...
                      {selectedMatch.client && (
                        <div className={styles.similarTeams}>
                          ...
                          <div className={styles.suggestionsGrid}>
                            {findSimilarTeams(selectedMatch.client).map(team => (
                              <button 
                                key={team.id}
                  ...
                  {selectedMatch.players?.client && (
                    <div className={styles.playerInfo}>
                      <p className={styles.playerName}>
                        {selectedMatch.players.client.firstName} {selectedMatch.players.client.lastName}
                      </p>
                      {selectedMatch.players.client.UTR && (
                        <p className={styles.playerUTR}>UTR: {selectedMatch.players.client.UTR}</p>
                      )}
                    </div>
                  )}
                </div>
                ...
                <div className={styles.teamDisplay}>
                  ...
                  {selectedMatch.opponent ? (
                    <p className={styles.teamNameDisplay}>{selectedMatch.opponent}</p>
                  ) : (
                    <p className={styles.missingTeam}>Unknown team</p>
                  )}
                  
                  {/* Show suggestion if team is unknown */}
                  {(!selectedMatch.opponent || !teams.some(t => t.name === selectedMatch.opponent)) && (
                    <div className={styles.teamSuggestion}>
                      <p>Suggested: {getSuggestedTeam(selectedMatch.opponent, false)}</p>
                      <button 
                        className={styles.applySuggestion}
                        onClick={() => setOpponentTeam(getSuggestedTeam(selectedMatch.opponent, false))}
                      >
                        Apply Suggestion
                      </button>
                      ...
                      {selectedMatch.opponent && (
                        <div className={styles.similarTeams}>
                          <p className={styles.similarTitle}>Similar Teams:</p>
                          <div className={styles.suggestionsGrid}>
                            {findSimilarTeams(selectedMatch.opponent).map(team => (
                              <button 
                                key={team.id}
                                className={styles.similarTeamBtn} 
                  ...
                  {selectedMatch.players?.opponent && (
                    <div className={styles.playerInfo}>
                      <p className={styles.playerName}>
                        {selectedMatch.players.opponent.firstName} {selectedMatch.players.opponent.lastName}
                      </p>
                      {selectedMatch.players.opponent.UTR && (
                        <p className={styles.playerUTR}>UTR: {selectedMatch.players.opponent.UTR}</p>
                      )}
                    </div>
                  )}
                </div>
              ...
              {(selectedMatch.score || (selectedMatch.points && selectedMatch.points.length > 0) || (selectedMatch.pointsJson && selectedMatch.pointsJson.length > 0)) && (
                <div className={styles.statisticsSection}>
                      ...
                      <div className={styles.scoreBody}>
                        <div className={styles.scoreRow}>
                          <span className={styles.scoreTeamName}>
                            {selectedMatch.client || selectedMatch.teams?.clientTeam || 'Client'}
                          </span>
                          {selectedMatch.score.sets && selectedMatch.score.sets.map((set, index) => (
                        ...
                        <div className={styles.scoreRow}>
                          <span className={styles.scoreTeamName}>
                            {selectedMatch.opponent || selectedMatch.teams?.opponentTeam || 'Opponent'}
                          </span>
                          {selectedMatch.score.sets && selectedMatch.score.sets.map((set, index) => (
                            <span key={index} className={styles.scoreCell}>
                              {set.opponent}
                  ...
                  {selectedMatch.points && selectedMatch.points.length > 0 && !selectedMatch.score && (
                    <div>
                      ...
                      {(() => {
                        ...
                        const clientTeam = lastPoint.clientTeam || selectedMatch.client || '';
                        const opponentTeam = lastPoint.opponentTeam || selectedMatch.opponent || '';
                        ...
                        points.forEach(point => {
                          if (point.setNum && point.player1GameScore !== undefined && point.player2GameScore !== undefined) {
                            ...
                            if (point.player1GameScore > setScores[setNum].client) {
                              setScores[setNum].client = point.player1GameScore;
                            }
                            
                            if (point.player2GameScore > setScores[setNum].opponent) {
                              setScores[setNum].opponent = point.player2GameScore;
                            }
                          }
                        });
                        
                        // Convert to array
                        const setsArray = Object.keys(setScores).map(setNum => ({
                        ...
                        if (setsArray.length > 0) {
                          setsArray.forEach(set => {
                            if (set.client > set.opponent) finalScores.client++;
                            if (set.opponent > set.client) finalScores.opponent++;
                          });
                        }
                          ...
                          <div>
                            ...
                            <div className={styles.statsContainer}>
                              ...
                              <div className={styles.statsTable}>
                                ...
                                {(() => {
                                  // Calculate aces
                                  const clientAces = points.filter(p => p.isAce && p.serverName === clientPlayer).length;
                                  const opponentAces = points.filter(p => p.isAce && p.serverName === opponentPlayer).length;
                                  
                                  // Calculate double faults
                                  const clientDoubleFaults = points.filter(p => 
                                    p.serveResult === "Double Fault" && p.serverName === clientPlayer
                                  ).length;
                                  const opponentDoubleFaults = points.filter(p => 
                                    p.serveResult === "Double Fault" && p.serverName === opponentPlayer
                                  ).length;
                                  
                                  // Calculate first serve percentage
                                  const clientFirstServes = points.filter(p => p.serverName === clientPlayer && p.firstServeIn !== undefined).length;
                                  const clientFirstServesIn = points.filter(p => p.serverName === clientPlayer && p.firstServeIn === 1).length;
                                  
                                  const opponentFirstServes = points.filter(p => p.serverName === opponentPlayer && p.firstServeIn !== undefined).length;
                                  const opponentFirstServesIn = points.filter(p => p.serverName === opponentPlayer && p.firstServeIn === 1).length;
                                  
                                  const clientFirstServePercentage = clientFirstServes > 0 ? 
                                    Math.round((clientFirstServesIn / clientFirstServes) * 100) : 0;
                                  
                                  const opponentFirstServePercentage = opponentFirstServes > 0 ? 
                                    Math.round((opponentFirstServesIn / opponentFirstServes) * 100) : 0;
                                  
                                  // Calculate points won
                                  const clientPointsWon = points.filter(p => p.pointWonBy === clientPlayer).length;
                                  const opponentPointsWon = points.filter(p => p.pointWonBy === opponentPlayer).length;
                                  
                                  // Return the stats UI
                                  return (
                                    <>
                                      <div className={styles.statsRow}>
                                        <span className={styles.statLabel}>Points Won</span>
                                        <span className={styles.statValue}>{clientPointsWon}</span>
                                        <span className={styles.statValue}>{opponentPointsWon}</span>
                                      </div>
                                      <div className={styles.statsRow}>
                                        <span className={styles.statLabel}>Aces</span>
                                        <span className={styles.statValue}>{clientAces}</span>
                                        <span className={styles.statValue}>{opponentAces}</span>
                                      </div>
                                      <div className={styles.statsRow}>
                                        <span className={styles.statLabel}>Double Faults</span>
                                        <span className={styles.statValue}>{clientDoubleFaults}</span>
                                        <span className={styles.statValue}>{opponentDoubleFaults}</span>
                                      </div>
                                      <div className={styles.statsRow}>
                                        <span className={styles.statLabel}>First Serve %</span>
                                        <span className={styles.statValue}>{clientFirstServePercentage}%</span>
                                        <span className={styles.statValue}>{opponentFirstServePercentage}%</span>
                                      </div>
                                    </>
                                  );
                                })()}
                  ...
                  {selectedMatch.pointsJson && selectedMatch.pointsJson.length > 0 && !selectedMatch.score && (
                    <div>
                      {/* Extract players from pointsJson data */}
                      {(() => {
                        const points = selectedMatch.pointsJson;
                        const clientTeam = selectedMatch.teams?.clientTeam || selectedMatch.client || '';
                        const opponentTeam = selectedMatch.teams?.opponentTeam || selectedMatch.opponent || '';
                        
                        // Find player names
                        const clientPlayers = [...new Set(points.filter(p => 
                          p.clientTeam && p.player1Name).map(p => p.player1Name))];
                        const opponentPlayers = [...new Set(points.filter(p => 
                          p.opponentTeam && p.player2Name).map(p => p.player2Name))];
                        
                        const clientPlayer = clientPlayers.length > 0 ? clientPlayers[0] : '';
                        const opponentPlayer = opponentPlayers.length > 0 ? opponentPlayers[0] : '';
                          ...
                          <div>
                            <div className={styles.scoreContainer}>
                              ...
                              <div className={styles.scoreBody}>
                                <div className={styles.scoreRow}>
                                  <span className={styles.scoreTeamName}>
                                    {clientTeam} {clientPlayer ? `(${clientPlayer})` : ''}
                                  </span>
                                  {setsArray.map((set, index) => (
                                    <span key={index} className={styles.scoreCell}>
                                      {set.client}
                                ...
                                <div className={styles.scoreRow}>
                                  <span className={styles.scoreTeamName}>
                                    {opponentTeam} {opponentPlayer ? `(${opponentPlayer})` : ''}
                                  </span>
                                  {setsArray.map((set, index) => (
                                    <span key={index} className={styles.scoreCell}>
                                      {set.opponent}
                            ...
                            <div className={styles.statsContainer}>
                              ...
                              <div className={styles.statsTable}>
                                ...
                                {(() => {
                                  // Calculate aces
                                  const clientAces = points.filter(p => p.isAce === true && p.serverName === clientPlayer).length;
                                  const opponentAces = points.filter(p => p.isAce === true && p.serverName === opponentPlayer).length;
                                  
                                  // Calculate double faults
                                  const clientDoubleFaults = points.filter(p => 
                                    p.serveResult === "Double Fault" && p.player1Name === clientPlayer
                                  ).length;
                                  const opponentDoubleFaults = points.filter(p => 
                                    p.serveResult === "Double Fault" && p.player2Name === opponentPlayer
                                  ).length;
                                  
                                  // Calculate first serve percentage
                                  const clientFirstServes = points.filter(p => p.player1Name === clientPlayer && p.firstServeIn !== undefined).length;
                                  const clientFirstServesIn = points.filter(p => p.player1Name === clientPlayer && p.firstServeIn === 1).length;
                                  
                                  const opponentFirstServes = points.filter(p => p.player2Name === opponentPlayer && p.firstServeIn !== undefined).length;
                                  const opponentFirstServesIn = points.filter(p => p.player2Name === opponentPlayer && p.firstServeIn === 1).length;
                                  ...
                                  const clientWinners = points.filter(p => 
                                    p.lastShotResult === "Winner" && p.lastShotHitBy === clientPlayer
                                  ).length;
                                  const opponentWinners = points.filter(p => 
                                    p.lastShotResult === "Winner" && p.lastShotHitBy === opponentPlayer
                                  ).length;
                                  
                                  const clientErrors = points.filter(p => 
                                    p.lastShotResult === "Error" && p.lastShotHitBy === clientPlayer
                                  ).length;
                                  const opponentErrors = points.filter(p => 
                                    p.lastShotResult === "Error" && p.lastShotHitBy === opponentPlayer
                                  ).length;
                                  ...
                                  return (
                                    <>
                                      ...
                                      <div className={styles.statsRow}>
                                        ...
                                        <span className={styles.statValue}>{clientWinners}</span>
                                        <span className={styles.statValue}>{opponentWinners}</span>
                                      </div>
                                      <div className={styles.statsRow}>
                                        <span className={styles.statLabel}>Unforced Errors</span>
                                        <span className={styles.statValue}>{clientErrors}</span>
                                        <span className={styles.statValue}>{opponentErrors}</span>
                                      </div>
                                      <div className={styles.statsRow}>
                                        <span className={styles.statLabel}>Aces</span>
                                        <span className={styles.statValue}>{clientAces}</span>
                  ...
                  {selectedMatch.statistics && (
                    <div className={styles.statsContainer}>
                      ...
                      <div className={styles.statsTable}>
                        <div className={styles.statsRow}>
                          <span className={styles.statLabel}>Aces</span>
                          <span className={styles.statValue}>{selectedMatch.statistics.client?.aces || '0'}</span>
                          <span className={styles.statValue}>{selectedMatch.statistics.opponent?.aces || '0'}</span>
                        </div>
                        <div className={styles.statsRow}>
                          <span className={styles.statLabel}>Double Faults</span>
                          <span className={styles.statValue}>{selectedMatch.statistics.client?.doubleFaults || '0'}</span>
                          <span className={styles.statValue}>{selectedMatch.statistics.opponent?.doubleFaults || '0'}</span>
                        </div>
                        <div className={styles.statsRow}>
                          <span className={styles.statLabel}>First Serve %</span>
                          <span className={styles.statValue}>{selectedMatch.statistics.client?.firstServePercentage || '0'}%</span>
                          <span className={styles.statValue}>{selectedMatch.statistics.opponent?.firstServePercentage || '0'}%</span>
                        </div>
                        <div className={styles.statsRow}>
                          <span className={styles.statLabel}>First Serve Points Won</span>
                          <span className={styles.statValue}>{selectedMatch.statistics.client?.firstServePointsWon || '0'}</span>
                          <span className={styles.statValue}>{selectedMatch.statistics.opponent?.firstServePointsWon || '0'}</span>
                        </div>
                        <div className={styles.statsRow}>
                          <span className={styles.statLabel}>Second Serve Points Won</span>
                          <span className={styles.statValue}>{selectedMatch.statistics.client?.secondServePointsWon || '0'}</span>
                          <span className={styles.statValue}>{selectedMatch.statistics.opponent?.secondServePointsWon || '0'}</span>
                        </div>
                        <div className={styles.statsRow}>
                          <span className={styles.statLabel}>Break Points Saved</span>
                          <span className={styles.statValue}>{selectedMatch.statistics.client?.breakPointsSaved || '0'}</span>
                          <span className={styles.statValue}>{selectedMatch.statistics.opponent?.breakPointsSaved || '0'}</span>
                        </div>
                      </div>
                    </div>
                  )}
                  
                  {!selectedMatch.statistics && !selectedMatch.points && !selectedMatch.pointsJson && selectedMatch.score && (
                    <div className={styles.noStatsMessage}>
                      <p>Detailed statistics for this match are not available.</p>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className={styles.matchInfo}>
                <p><strong>Date:</strong> {formatDate(selectedMatch.date || selectedMatch.matchDate)}</p>
                <p><strong>Collection:</strong> {selectedMatch.collection}</p>
                
                {selectedMatch.matchDetails && (
                  <div className={styles.additionalInfo}>
                    {selectedMatch.matchDetails.event && (
                      <p><strong>Event:</strong> {selectedMatch.matchDetails.event}</p>
                    )}
                    {selectedMatch.matchDetails.division && (
                      <p><strong>Division:</strong> {selectedMatch.matchDetails.division}</p>
                    )}
                    {selectedMatch.matchDetails.round && (
                      <p><strong>Round:</strong> {selectedMatch.matchDetails.round}</p>
                    )}
                    {selectedMatch.matchDetails.surface && (
                      <p><strong>Surface:</strong> {selectedMatch.matchDetails.surface}</p>
                    )}
                  </div>
                )}
                
                <p><strong>Match ID:</strong> {selectedMatch.id}</p>
