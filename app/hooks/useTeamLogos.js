  ...
  useEffect(() => {
    const fetchLogos = async () => {
      try {
        ...
        const clientTeamData = allTeams.find(
          (team) => normalizeTeamName(team.name) === normalizedClientTeam
        )
        const opponentTeamData = allTeams.find(
          (team) => normalizeTeamName(team.name) === normalizedOpponentTeam
        )
